...
static bool LoadImageRGBA(std::filesystem::path const& path, Vector<U32>& out, U32& w, U32& h) {
    ...
    hr = factory->CreateDecoderFromFilename(wpath.c_str(), nullptr, GENERIC_READ, WICDecodeMetadataCacheOnLoad, decoder.GetAddressOf());
    if (FAILED(hr)) {
        if (doUninit) CoUninitialize();
        return false;
    }
...
static void BlitWithPad(Vector<U32>& dst, U32 atlasW, U32 atlasH, U32 tx, U32 ty, U32 tile, U32 pad, Vector<U32> const& src) {
...
public:
    ...
    void Run(World* world, F32) override {
        ...
        if (m_AtlasTex == INVALID_INDEX) {
            U32 pad{2};
            Vector<std::string> paths{"assets/dirt.png","assets/grass_side.png","assets/grass_top.png","assets/stone.png"};
            Vector<Vector<U32>> imgs{}; imgs.resize(paths.size());
            Vector<U32> w{}; w.resize(paths.size());
            Vector<U32> h{}; h.resize(paths.size());
            ...
            AtlasConstants ac{}; ac.tilesX = tilesX; ac.tileSize = tile; ac.pad = pad; ac._pad0 = 0; ac.atlasW = atlasW; ac.atlasH = atlasH;
            m_Gfx->UpdateConstantBuffer(m_AtlasCB, &ac, sizeof(ac));
        }

        CameraConstants cam{};
        if (auto h{CameraManager::GetPrimaryCamera()}; h.valid()) {
            if (auto* c{world->GetComponent<Camera>(h)}) { cam.view = c->view; cam.projection = c->projection; cam.viewProjection = c->viewProjection; }
        ...
        ObjectConstants obj{}; obj.world = Math::Mat4::Identity;
