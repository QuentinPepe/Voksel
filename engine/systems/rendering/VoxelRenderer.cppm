...
static bool LoadImageRGBA(const std::filesystem::path &path, Vector<U32> &out, U32 &w, U32 &h) {
    ...
    if (FAILED(
        factory->CreateDecoderFromFilename(path.wstring().c_str(), nullptr, GENERIC_READ, WICDecodeMetadataCacheOnLoad,
    ...
    if (FAILED(
        conv->Initialize(frame.Get(), GUID_WICPixelFormat32bppRGBA, WICBitmapDitherTypeNone, nullptr, 0.0,
            WICBitmapPaletteTypeCustom))) return false;
    out.resize(static_cast<USize>(fw) * fh);
    if (FAILED(
        conv->CopyPixels(nullptr, fw * 4, static_cast<UINT>(out.size() * 4), reinterpret_cast<BYTE*>(out.data()))))
        return false;
    w = fw;
    h = fh;
    return true;
