...
private:
   ...
   U32 m_Selected{0};
   bool m_Built{false};
   constexpr static U32 kTotalSlots{9};
...
public:
   ...
   void Run(World* world, F32) override {
       ...
       if (scroll != 0.0f && count > 0) {
           if (scroll > 0) {
               ApplySelection(world, (m_Selected + 1) % count);
           } else {
               ApplySelection(world, (m_Selected + count - 1) % count);
           }
       }
   }
...
private:
   ...
   void BuildIfNeeded(World* world) {
       ...
       items.push_back(Item{Voxel::Dirt, AtlasTileFor(Voxel::Dirt), true});
       items.push_back(Item{Voxel::Grass, AtlasTileFor(Voxel::Grass), true});
       items.push_back(Item{Voxel::Stone, AtlasTileFor(Voxel::Stone), true});
       items.push_back(Item{Voxel::Log, AtlasTileFor(Voxel::Log), true});
       items.push_back(Item{Voxel::Leaves, AtlasTileFor(Voxel::Leaves), true});

       for (USize i{items.size()}; i < kTotalSlots; ++i) {
           items.push_back(Item{Voxel::Air, 0, false});
       }
       ...
       for (USize i{}; i < items.size(); ++i) {
           ...
           UIElementPtr img{};
           if (items[i].hasItem) {
               img = std::static_pointer_cast<UIImage>(m_UI->CreateImage());
               img->SetAnchor(AnchorPreset::Center);
               img->SetPivot({0.5f, 0.5f});
               img->SetSizeDelta({32.0f, 32.0f});
               std::static_pointer_cast<UIImage>(img)->SetTexture(ai->texture);
               std::static_pointer_cast<UIImage>(img)->SetUVRect(TileUV(items[i].tile, *ai));
               panel->AddChild(img);
           }

           auto countText{m_UI->CreateText("")};
           ...
           if (items[i].hasItem) {
               std::static_pointer_cast<UIText>(countText)->SetText("64");
           }
           ...
           s.voxel = items[i].v;
           s.count = items[i].hasItem ? 64 : 0;
           s.hasItem = items[i].hasItem;
           m_Slots.push_back(s);
       }
