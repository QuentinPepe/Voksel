...
private:
    ...
    std::atomic_bool m_CursorDirty{false};
    std::atomic_bool m_RawDirty{false};
    bool m_PendingCursorVisible = true;
    bool m_PendingCursorLocked = false;
    bool m_PendingRaw = false;
...
public:
    ...
    void SetCursorVisible(bool visible) {
        m_PendingCursorVisible = visible;
        m_CursorDirty = true;
    }

    void SetCursorLocked(bool locked) {
        m_PendingCursorLocked = locked;
        m_CursorDirty = true;
        if (locked) {
            m_FirstMouse = true;
        }
    ...
    void SetRawMouseMode(bool raw) {
        m_PendingRaw = raw;
        m_RawDirty = true;
    }

    void Flush() {
        if (m_CursorDirty.exchange(false)) {
            m_CursorVisible = m_PendingCursorVisible;
            m_CursorLocked = m_PendingCursorLocked;
            UpdateCursorMode();
        }
        if (m_RawDirty.exchange(false)) {
            GLFWwindow* glfwWindow = m_Window->GetGLFWHandle();
            if (glfwRawMouseMotionSupported()) {
                glfwSetInputMode(glfwWindow, GLFW_RAW_MOUSE_MOTION, m_PendingRaw ? GLFW_TRUE : GLFW_FALSE);
                m_InputManager->SetRawMouseMode(m_PendingRaw);
                Logger::Debug(LogInput, "Raw mouse mode {}", m_PendingRaw ? "enabled" : "disabled");
            } else {
